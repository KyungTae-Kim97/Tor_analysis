...
data_size_per_class = sys.argv[3]
num_classes = int(sys.argv[4])
result_file = sys.argv[5]

df_train= pd.read_csv(file_dataframe1)
df_train = df_train.fillna(0) 

df_test= pd.read_csv(file_dataframe2)
df_test = df_test.fillna(0)  

X_train = df_train.iloc[:, :-1].values  

y_train = df_train.iloc[:, -1].values  
num_columns = df_train.shape[1] -1
...
f.write(f"train is {file_dataframe1}\n")
f.write(f"test is {file_dataframe2}\n")
